      ...
      {#each accounts as account}
        <div class="bg-fleeca-card rounded-lg border border-fleeca-border p-4 hover:shadow-card transition-all">
          <div class="flex justify-between items-center mb-3">
            <h4 class="text-fleeca-text font-medium">{account?.name}</h4>
            <div class="flex space-x-2">
              <button 
                aria-label={$translations.manage_members || 'Manage Members'}
              ...
              <button 
                aria-label={$translations.edit_acc_name || 'Change Account Name'}
                class="w-8 h-8 rounded-full bg-fleeca-bg flex items-center justify-center text-fleeca-text-secondary hover:text-fleeca-text transition-colors"
              ...
              <button 
                aria-label={$translations.delete_account || 'Delete Account'}
          ...
          <div class="text-fleeca-text-secondary text-sm">
            {$translations.account_id || 'Account ID'}: {account.id}
          </div>
        </div>
      {/each}
    </div>
