<script lang="ts">
  ...
  import AccountTransactionItem from "./Accounts/AccountTransactionItem.svelte";
  ...
  $: account =
    $activeAccount && $accounts && Array.isArray($accounts)
...
<section transition:fly={{
  ...
  {#if account}
          ...
          <div>
            ...
            >
              {account.type === "personal" || account.type === "Personal"
                ? "Personal Account"
                : account.type === "business" || account.type === "Business"
                  ? "Business Account"
                  : account.organisation || "Personal Account"}
            </h2>
    ...
    >
      {#if account.transactions && Array.isArray(account.transactions) && account.transactions.length > 0}
        {#if account.transactions.filter((item: { message: string, trans_id: string, receiver: string}) => item && item.message && typeof item.message === "string" && (item.message
              .toLowerCase()
              .includes((transSearch || "").toLowerCase()) || (item.trans_id && typeof item.trans_id === "string" && item.trans_id
                  .toLowerCase()
                  .includes((transSearch || "").toLowerCase())) || (item.receiver && typeof item.receiver === "string" && item.receiver
                  .toLowerCase()
                  .includes((transSearch || "").toLowerCase())))).length > 0}
          <div class="space-y-3 animate-fadeIn">
            {#each account.transactions.filter((item: { message: string, trans_id: string, receiver: string}) => item && item.message && typeof item.message === "string" && (item.message
                  .toLowerCase()
                  .includes((transSearch || "").toLowerCase()) || (item.trans_id && typeof item.trans_id === "string" && item.trans_id
                      .toLowerCase()
                      .includes((transSearch || "").toLowerCase())) || (item.receiver && typeof item.receiver === "string" && item.receiver
  ...
  {:else}
    <div class="flex-1 flex items-center justify-center">
      <div
        class="text-center py-12 px-6 bg-fleeca-card rounded-lg border border-fleeca-border animate-fadeIn max-w-md shadow-md"
      >
        <div
          class="w-20 h-20 mx-auto bg-fleeca-bg rounded-full flex items-center justify-center mb-6"
        >
        ...
        <p class="text-fleeca-text-secondary">
          Choose a card from the left to view details and manage your finances
        </p>
      </div>
    </div>
  {/if}
